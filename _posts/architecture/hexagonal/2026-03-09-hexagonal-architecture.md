...
이 글을 읽기 전에 `DDD`에 대한 기본적인 이해가 있으면 좋다.  
Hexagonal Architecture는 도메인을 중심으로 설계를 하는 구조이기 때문에  
DDD의 개념을 알고 있으면 내용이 훨씬 쉽게 이해된다.

아래 글에서 DDD의 핵심 개념을 먼저 정리했으니  
...
- DDD란? 시리즈
  ...
  - [hexagonal architecture](https://angrypig123.github.io/posts/ddd-hexagonalarchitecture/)
...
이 글은 `Hexagonal Architecture`를 학습하면서 정리한 내용이다.
...
`DDD`의 주요 개념과 `Hexagonal Architecture`의 관계를 정리했다면,
이제 실제 설계에서 자주 등장하는 추가 개념들을 정리해보려고 한다.
...
구조를 통해 도메인을 외부로부터 분리한다.

또한 시스템이 커질수록 여러 개의 `Bounded Context`가 생기게 되는데,
이때 `Context` 간 관계를 정의하는

- `Context Map`

개념도 중요해진다.

다음 글부터는 위 개념들을 하나씩 정리해보려고 한다.
...
![Hexagonal Architecture 호출 흐름 예시](/assets/img/architecture/hexagonal-architecture-call-example.png)
